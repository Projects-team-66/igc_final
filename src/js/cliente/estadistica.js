...
const getEstadisticas = async () => {
    const url = `/igc_final/API/detalle/estadistica`
    ...
    if(data){
        if(chartCliente.data.datasets[0]) {
            ...
            data.forEach(r => {
                chartCliente.data.labels.push(r.asistencia_alumno);
                chartCliente.data.datasets[0].data.push(r.asistencia_curso);
                chartCliente.data.datasets[0].data.push(r.total_asistencia);
